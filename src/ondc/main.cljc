(ns ondc.main
  "Kotodama WASM entrypoint for the Ondc clean-room actor (L4) — Clojure port.
  ...
  facts (`ondc.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"ondc.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ondc-compat" :tier tier :entities entities} 200])
